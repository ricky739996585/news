...
public interface IOperations<T extends Serializable, U extends Serializable> {
	...
	void insertByBatch(List<T> list, @Param("tableName") String tableName);
	...
	List<U> getList(@Param("condition") LinkedHashMap<String, String> condition, @Param("offset") int offset, @Param("limit") int limit,
			@Param("order") String order, @Param("field") String field, @Param("tableName") String tableName);
